% 2017 EC503 Project virigi gknn
%% load data
...
for k = 1:19:150
    ...
    nor_dis = avg_dis(label == 2);
    A = 1:size(data,1);
    scatter(A(label == 2), nor_dis, 'b');
    hold on
    ...
    one_prediction = one_prediction+1;
    one_prediction(one_anormly_inx) = 1;
    fprintf('%d-nn', k);
    oneconf = confusionmat(one_prediction, label);
    printmat(oneconf, '1 std Confusion Matrix', 'PredAnomaly PredNormal', 'GTAnomaly GTNormal');
    
    
    onerate(count,1) = oneconf(1,1);
    onerate(count,2) = oneconf(1,2);
    onerate(count,3) = oneconf(2,1);
    onerate(count,4) = oneconf(2,2);
    ...
    two_prediction = two_prediction+1;
    two_prediction(two_anormly_inx) = 1;
    twoconf = confusionmat(two_prediction, label);
    printmat(twoconf, '2 std Confusion Matrix', 'PredAnomaly PredNormal', 'GTAnomaly GTNormal');
    
    tworate(count,1) = twoconf(1,1);
    tworate(count,2) = twoconf(1,2);
    tworate(count,3) = twoconf(2,1);
    tworate(count,4) = twoconf(2,2);
